...
export default function PecsBoard({
    ...
    const syncSentence = async () => {
      ...
      if (alunoData && alunoData.id) {
        try {
          ...
          await updateDoc(pacienteRef, { currentSentence: sentence });
  ...
  useEffect(() => {
    ...
    const alunoData = savedConfig ? JSON.parse(savedConfig) : null;
    if (alunoData && alunoData.id) {
      const q = query(collection(db, "cartoes_customizados"), where("paciente_id", "==", alunoData.id));
  ...
  const verificarBloqueioSpam = (tempoCooldown = 350): boolean => {
    ...
    if (agora - ultimoCliqueRef.current < tempoCooldown) return true;
  ...
  const handleRemoveIndex = (idx: number) => { if (verificarBloqueioSpam(350)) return; setSentence((prev) => prev.filter((_, i) => i !== idx)); };
  const handleClear = () => { if (verificarBloqueioSpam(400)) return; setSentence([]); };
  const handleBackspace = () => { if (verificarBloqueioSpam(350)) return; setSentence((prev) => prev.slice(0, -1)); };
  const handleNavHome = () => { if (verificarBloqueioSpam(400)) return; setHistoryStack(['home']); };
  const handleNavBack = () => { if (verificarBloqueioSpam(400)) return; setHistoryStack((prev) => prev.length > 1 ? prev.slice(0, -1) : prev); };
  ...
  const handleOpenPanelSecure = () => {
    ...
    if (parseInt(resposta || '') === (num1 + num2)) onOpenPanel();
    else alert("Acesso recusado.");
  };
  ...
  const sidebarItems = [
    ...
    { id: 'beber', label: 'Beber', icon: <Lucide.CupSoda size={28} />, target: 'beber', activeColor: 'bg-cyan-500 text-white border-cyan-600 shadow-lg', inactiveColor: 'bg-cyan-100 text-cyan-700 hover:bg-cyan-200 border-cyan-200' },
  ...
  return (
    <motion.div className="flex-1 flex flex-col w-full h-[100dvh] mx-auto bg-white shadow-2xl relative overflow-hidden">
      <SentenceBar sentence={sentence} onRemoveIndex={handleRemoveIndex} onClear={handleClear} onBackspace={handleBackspace} onSpeak={handleSpeakSentence} onNavHome={handleNavHome} onNavBack={handleNavBack} isSpeaking={isSpeaking} historyLength={historyStack.length} />
      ...
      <div className="flex-1 flex overflow-hidden">
          ...
          {sidebarItems.map((item) => {
            ...
            return (
              <button key={item.id} onClick={() => handleSidebarClick(item.target)} className="flex flex-col items-center justify-center gap-1 group w-full px-1">
                <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-2xl flex items-center justify-center border-2 transition-all ${isActive ? item.activeColor : item.inactiveColor}`}>
        ...
        <div className="flex-1 p-3 sm:p-6 overflow-y-auto bg-slate-50">
          {(() => {
            const cardsFiltrados = [...cards, ...customCards].filter(card => {
              if (currentCategory === 'home') return card.isEssential === true || card.isCustom === true;
            ...
            if (cardsFiltrados.length === 0) {
              return (
                <div className="h-full flex flex-col items-center justify-center text-center p-8 opacity-70">
                  ...
                  <h2 className="text-2xl font-bold text-slate-700 mb-3">Bem-vindo ao Conecta Pecs!</h2>
                  <p className="text-slate-600 max-w-md">Selecione uma categoria na barra lateral esquerda para começar. Depois, toque nos cartões para formar frases e use o botão de voz para falar.</p>
                </div>
              );
            }

            return (
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 2xl:grid-cols-10 gap-3 sm:gap-4 pb-10">
      ...
      <footer className="shrink-0 px-3 sm:px-6 py-3 border-t border-slate-200 flex items-center justify-between bg-slate-50 h-14">
        <div className="flex items-center gap-2 truncate">
          <span className={`w-3 h-3 rounded-full shadow-sm animate-pulse shrink-0 ${activeRole === 'therapist' ? 'bg-purple-500' : activeRole === 'parent' ? 'bg-indigo-500' : 'bg-emerald-500'}`} />
          <span className="text-[9px] sm:text-[10px] font-extrabold uppercase tracking-wider text-slate-600 truncate">
            {activeRole === 'therapist' ? `Terapeuta: ${username}` : activeRole === 'parent' ? `Responsável: ${username}` : `Estudante: ${username}`}
          </span>
        </div>
        <button onClick={handleOpenPanelSecure} className="flex items-center gap-1.5 text-[9px] sm:text-[11px] font-extrabold px-3 py-1.5 bg-white hover:bg-amber-50 text-amber-700 border-2 border-amber-200 rounded-xl transition-all shadow-sm active:scale-95">
